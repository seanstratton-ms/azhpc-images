...
if [[ $DISTRIBUTION == "azurelinux3.0" || $DISTRIBUTION == *"debian"* || "${NODE_TYPE:-azure-vm}" == "baremetal" ]]; then
    # Debian installs pmix via apt to system paths (not /opt/pmix/<ver>), so
    # pass bare --with-pmix and let OMPI locate the system pmix.
...
tar -xvf $TARBALL
cd $OMPI_FOLDER
if [[ "${NODE_TYPE:-azure-vm}" == "baremetal" || $DISTRIBUTION == *"debian"* ]]; then
    PMIX_FLAG="--with-pmix"
else
